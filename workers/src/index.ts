...
type AuditEvent = { ts: string; ok: boolean; email: string; workspace_id: string; workspace_name: string; reason: string; ip: string; user_agent: string };
...
function adminCookieName(): string { return "oidc_admin_session"; }
async function adminSignature(secret: string, payload: string): Promise<string> {
  const enc = new TextEncoder();
  const key = await crypto.subtle.importKey("raw", enc.encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  return b64url(await crypto.subtle.sign("HMAC", key, enc.encode(payload)));
}
async function createAdminSession(secret: string): Promise<string> {
  const payload = `${Math.floor(Date.now() / 1000)}.${randomToken(18)}`;
  return `${payload}.${await adminSignature(secret, payload)}`;
}
async function validAdminSession(request: Request, secret: string): Promise<boolean> {
  ...
  if (!Number.isFinite(ts) || !nonce || Math.floor(Date.now() / 1000) - ts > 12 * 3600) return false;
  const expected = await adminSignature(secret, `${tsRaw}.${nonce}`);
  return safeEqual(sig, expected);
}
function adminLoginPage(error = ""): Response {
  const err = error ? `<div class="err">${esc(error)}</div>` : "";
  return html(`<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin Login</title><style>
:root{color-scheme:light dark;--bg:#f6f7fb;--card:rgba(255,255,255,.78);--text:#111318;--muted:#697386;--line:rgba(15,23,42,.10);--accent:#5e6ad2;--accent2:#10a37f;--danger:#dc2626;--input:#fff}@media(prefers-color-scheme:dark){:root{--bg:#08090a;--card:rgba(16,17,20,.78);--text:#f7f8f8;--muted:#8a8f98;--line:rgba(255,255,255,.10);--input:rgba(255,255,255,.04)}}*{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;padding:28px;background:radial-gradient(760px 460px at 20% -10%,rgba(94,106,210,.22),transparent 60%),radial-gradient(620px 420px at 92% 10%,rgba(16,163,127,.16),transparent 55%),linear-gradient(180deg,var(--bg),var(--bg));font-family:-apple-system,BlinkMacSystemFont,"SF Pro Display","SF Pro Text","Helvetica Neue",Arial,sans-serif;color:var(--text);letter-spacing:-.012em}.shell{width:min(960px,100%);display:grid;grid-template-columns:1fr 420px;gap:22px;align-items:stretch}.hero,.card{border:1px solid var(--line);background:var(--card);backdrop-filter:blur(24px);box-shadow:0 30px 90px rgba(15,23,42,.14),inset 0 1px 0 rgba(255,255,255,.18);border-radius:30px}.hero{padding:34px;display:flex;flex-direction:column;justify-content:space-between;min-height:460px;overflow:hidden;position:relative}.hero:after{content:"";position:absolute;right:-120px;bottom:-140px;width:360px;height:360px;border-radius:50%;background:linear-gradient(135deg,rgba(94,106,210,.38),rgba(16,163,127,.22));filter:blur(4px)}.brand{display:flex;align-items:center;gap:12px;position:relative;z-index:1}.mark{width:44px;height:44px;border-radius:14px;background:linear-gradient(135deg,var(--accent),var(--accent2));box-shadow:0 16px 36px rgba(94,106,210,.22)}.brand b{display:block;font-size:15px;font-weight:700}.brand span{display:block;margin-top:2px;color:var(--muted);font-size:12px}.hero h1{position:relative;z-index:1;margin:0 0 12px;font-size:48px;line-height:.96;letter-spacing:-2px}.hero p{position:relative;z-index:1;margin:0;color:var(--muted);font-size:15px;line-height:1.65;max-width:440px}.chips{position:relative;z-index:1;display:flex;gap:8px;flex-wrap:wrap;margin-top:24px}.chip{border:1px solid var(--line);background:rgba(255,255,255,.06);border-radius:999px;padding:8px 10px;color:var(--muted);font-size:12px;font-weight:600}.card{padding:30px}.card h2{margin:0 0 8px;font-size:24px;letter-spacing:-.8px}.card .sub{margin:0 0 24px;color:var(--muted);font-size:14px;line-height:1.5}label{display:block;margin:0 0 8px;color:var(--muted);font-size:12px;font-weight:700}input{width:100%;height:48px;border:1px solid var(--line);background:var(--input);color:var(--text);border-radius:14px;padding:0 14px;font:inherit;font-size:15px;outline:none}input:focus{border-color:rgba(94,106,210,.62);box-shadow:0 0 0 4px rgba(94,106,210,.14)}button{width:100%;height:48px;border:0;border-radius:14px;margin-top:16px;background:linear-gradient(135deg,var(--accent),#7474ff);color:#fff;font:inherit;font-weight:800;cursor:pointer;box-shadow:0 14px 34px rgba(94,106,210,.24)}.err{margin:0 0 16px;padding:12px 13px;border:1px solid rgba(220,38,38,.22);background:rgba(220,38,38,.10);color:var(--danger);border-radius:14px;font-size:13px}.foot{margin-top:18px;color:var(--muted);font-size:12px;line-height:1.5}@media(max-width:820px){.shell{grid-template-columns:1fr}.hero{min-height:280px}.hero h1{font-size:38px}}
</style></head><body><main class="shell"><section class="hero"><div class="brand"><div class="mark"></div><div><b>ChatGPT SSO</b><span>OIDC Admin Console</span></div></div><div><h1>管理入口</h1><p>使用管理员密码进入 Workspace、Client、回调地址与密钥管理界面。</p><div class="chips"><span class="chip">Secure session cookie</span><span class="chip">12h session</span><span class="chip">No browser basic-auth prompt</span></div></div></section><section class="card"><h2>Admin Login</h2><p class="sub">输入 Worker secret 中配置的管理员密码。</p>${err}<form method="post" action="/admin/login"><label>Admin password</label><input name="password" type="password" autocomplete="current-password" placeholder="••••••••" required autofocus><button type="submit">Continue →</button></form><div class="foot">也仍支持 Basic Auth 作为自动化/脚本访问方式。</div></section></main></body></html>`);
}
...
async function requireAdmin(request: Request, env: Env): Promise<Response | undefined> {
  ...
  if (auth && safeEqual(auth.secret, password)) return undefined;
  if (await validAdminSession(request, password)) return undefined;
...
async function adminLogin(request: Request, env: Env): Promise<Response> {
  ...
  if (limited) return adminLoginPage("Too many attempts. Please try again later.");
  const form = await request.formData();
  const provided = String(form.get("password") || "");
  if (!safeEqual(provided, password)) return adminLoginPage("管理员密码不正确 / Invalid admin password");
  const token = await createAdminSession(password);
  return new Response(null, { status: 302, headers: { "location": new URL("/admin", request.url).toString(), "set-cookie": `${adminCookieName()}=${token}; Path=/admin; HttpOnly; Secure; SameSite=Lax; Max-Age=43200` } });
}
function adminLogout(request: Request): Response {
  return new Response(null, { status: 302, headers: { "location": new URL("/admin/login", request.url).toString(), "set-cookie": `${adminCookieName()}=; Path=/admin; HttpOnly; Secure; SameSite=Lax; Max-Age=0` } });
}
...
async function auditLogin(request: Request, env: Env, ws: Workspace, email: string, ok: boolean, reason: string): Promise<void> {
  const key = "audit:logins";
  const raw = await env.OIDC_KV.get(key);
  const events = raw ? JSON.parse(raw) as AuditEvent[] : [];
  events.unshift({ ts: new Date().toISOString(), ok, email: email || "-", workspace_id: ws.id, workspace_name: ws.name, reason, ip: clientIp(request), user_agent: (request.headers.get("user-agent") || "").slice(0, 120) });
  await env.OIDC_KV.put(key, JSON.stringify(events.slice(0, 100)));
}
async function loginAuditEvents(env: Env): Promise<AuditEvent[]> {
  const raw = await env.OIDC_KV.get("audit:logins");
...
async function adminSave(request: Request, env: Env): Promise<Response> {
  ...
  return Response.redirect(new URL("/admin/workspaces", request.url).toString(), 302);
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (url.pathname.startsWith("/admin")) {
        if (url.pathname === "/admin/login" && request.method === "GET") return adminLoginPage();
        if (url.pathname === "/admin/login" && request.method === "POST") return adminLogin(request, env);
        if (url.pathname === "/admin/logout") return adminLogout(request);
        const denied = await requireAdmin(request, env);
        if (denied) return denied;
        if (url.pathname === "/admin/preview" && request.method === "GET") return adminPreviewPage(env, url.searchParams.get("id") || "");
        if (url.pathname === "/admin" && request.method === "GET") return adminHomePage(env);
        if (url.pathname === "/admin/workspaces" && request.method === "GET") return adminPage(env, url.searchParams.get("edit") || "");
      ...
      if (url.pathname === "/authorize" && request.method === "POST") {
        ...
        if (inviteError) { await auditLogin(request, env, ws, email, false, inviteError); return loginPage(ws, keep, inviteError); }
        if (params.response_type !== "code") return json({ error: "unsupported_response_type" }, 400);
        await auditLogin(request, env, ws, email, true, "invite accepted");
